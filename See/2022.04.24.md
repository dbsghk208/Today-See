...
from 테이블
where 1=1
<if test= "(data.search_trans_id != '' and data.search_trans_id != null)"> 
 and trans_id = trim(#{data.search_trans_id})
...
- 뉴렉처 님의 Servelt 강의는 정말 좋다.  
진도를 나가는데 있어 개발할때 도움이 되어서 계속 꾸준히 들어야 하는 명강의이다.  
    
- SQLD 는 이직할때 도움이 되고자 따려고 한다.
<br><br>

다른 곳으로 가고는 싶은데 그걸 너무 막연히 미루는것은 아닌지   
그 부분에 대해서도 다시 생각해보게 되었다.     
정말 다른데로 가서 개발쪽으로 실력을 쌓고 싶다.   
지금은 0이니깐 나에게 조금이라도 도움이 되지만,    
진짜 벗어나고 싶으니까 이직을 해야한다.
<br>
